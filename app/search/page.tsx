            ...
            <div className="max-w-7xl mx-auto">
                <header className="mb-16">
                    <div className="flex items-center gap-4 mb-4">
                        ...
                        <h1 className="text-sm font-black tracking-[0.4em] uppercase opacity-50">Search Results</h1>
                    </div>
                ...
                {isLoading ? (
                    <div className="flex flex-col items-center justify-center py-40 gap-4">
                        ...
                        <p className="text-xs tracking-[0.3em] uppercase opacity-40">Scanning Global Archives...</p>
                    </div>
                ) : results.length > 0 ? (
                        ...
                        {results.map((item, idx) => (
                            <article key={idx} className="group cursor-pointer flex flex-col">
                                <div className="aspect-[4/5] overflow-hidden mb-6 bg-gray-200 relative">
                                    <img
                                        src={item.image || 'https://images.unsplash.com/photo-1558769132-cb1aea9f3dbc?w=600'}
                                        alt={item.title}
                                        onError={(e) => {
                                ...
                                <div className="space-y-3">
                                    <div className={`text-[10px] font-black tracking-widest uppercase ${showDarkMode ? 'text-vox-red' : 'text-gray-500'}`}>
                                        {item.date} — {item.author}
                                    </div>
                                    <h3 className="text-2xl font-serif leading-tight group-hover:text-vox-red transition-colors">
                                        {item.title}
                                    </h3>
                                    <p className={`text-sm leading-relaxed ${showDarkMode ? 'text-gray-400' : 'text-gray-600'} line-clamp-2 font-light`}>
                                        {item.description}
                                    </p>
                                </div>
                            </article>
                    ...
                    <div className="text-center py-40 space-y-6">
                        <p className="text-2xl font-serif italic opacity-40">No records found matching your voice.</p>
                        <p className="text-sm text-gray-500">다른 키워드로 검색하거나 트렌드 아카이브를 확인해 보세요.</p>
                    </div>
                )}
            </div>
        </div>
